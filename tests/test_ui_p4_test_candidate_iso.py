...
CONFIG_PATH = (
    PROJECT_ROOT
    / "config/iso/ui-p4-intermission-first-five-atlas-test-build.json"
)
COMPONENT_MANIFEST_PATH = (
    PROJECT_ROOT
    / "manifests/ui-p4-intermission-first-five-atlas-test-validation.json"
)
RUNTIME_MANIFEST_PATH = (
    PROJECT_ROOT
    / "manifests/ui-p4-intermission-first-five-atlas-test-runtime-validation.json"
)


class UiP4TestCandidateIsoTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_static_iso_report_and_manifest_are_exact(self):
        if not (
            PROJECT_ROOT / self.config["output"]["path"]
        ).is_file():
            self.skipTest(
        ...
        self.assertEqual(
            self.runtime["iso_build"]["output"],
            {
                "size": 3758456832,
                "sha256": (
                    "24b793d68b802bb36ae38ec47fcfe7d4b8d3f79177b006db"
                    "834ad821c62cd8cc"
                ),
            },
        )
    ...
    def test_runtime_boundary_is_explicit(self):
        self.assertEqual(
            self.runtime["status"],
            (
                "static_integrated_ui_p4_intermission_first_five_atlas_test_"
