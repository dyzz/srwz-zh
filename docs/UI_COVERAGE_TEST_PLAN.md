...
| 幕间主菜单、系统选项与编成入口 | 121 | 养、删、编、览 | 静态文本已进入组合 ISO；幕间标题、`新規編成` 与 `バザー` atlas 已有隔离映射 canary、待运行归属 |
| 人物、机体与武器信息页骨架 | 128 | 养、减、效、览、陆 | 静态文本及 1,307 个名称字段已进入 P2 组合 ISO；另有中文信息页 atlas 独立候选，仍待运行归属 |
| 战场指令、条件与战况页面 | 80 | 陆 | 静态文本已进入组合 ISO；`COMMAND MENU` atlas 已有隔离映射 canary、待运行归属 |
| 结算、升级与出击确认 | 52 | 0 | 静态文本已进入组合 ISO；`新規編成` atlas 已有隔离 canary，结算与出击路线待运行验证 |
...
no-op。`chunk 2 / record 0 / picture 0` 先完成了只清除顶行 `SHIP` 的
隔离映射 canary，现又以该前像和 mask 为边界生成首个中文 `机体` 候选。
人物／机体／武器信息页仍没有逐屏运行归属，因此该中文组件仍是
`runtime_mapping_pending`，不能视作已进入游戏的正式 atlas。
...
| 4 | `COMMAND MENU / FORMATION / BONUS / HIT&AWAY / HP / EN` | 战场指令、结算 | 隔离 `COMMAND MENU` 清除 canary 静态 ISO 已锁定，未做运行映射 |
| 5 | `バザー / 購入 / 売却 / 強化パーツ / アイテム` | 商店／交易 | 隔离大号 `バザー` 清除 canary 静态 ISO 已锁定，未做运行映射 |
| 6 | `インターミッション / オプション / 小隊編成 / データ管理` | 幕间、编成入口 | 隔离幕间标题清除 canary 静态 ISO 已锁定，未做运行映射 |
| 7 | `Event No / Leader / Pilot / 新規編成 / リザーブへ` | 编成、出击 | 隔离 `新規編成` 清除 canary 静态 ISO 已锁定，未做运行映射 |
...
信息页中文生产配置为 `config/assets/ui-info-atlas-zh.json`，唯一译文由
`corpus/zh/ui-atlas/info-v1.json` 所有。构建器逐字节复建上述擦除前像，
使用锁定的 LXGW 字体和原图已有灰度调色板，在 `49×16` mask 内增加 318 个
文字像素；相对原图精确变化 421 个像素／183 个 archive byte，TIM2 回读
RGBA 与渲染目标一致。独立 ISO SHA-256 为
`d31f3d3dbffc59da595b2d27bb516efec34af12426bda2b3d6f2a67ffdb9ddd0`。

六张隔离 ISO（五张基础定位图及一张信息页中文候选）都只替换
`KURODATA/KVMDATA.BIN`，完整归档等长，65 个未替换成员 byte-exact、零
LBA 位移。下一轮分别检查至少两台机体的信息子页、两台单位的战场指令菜单、
可访问的商店页、首个幕间主菜单和编成／出击页。运行矩阵的信息页用例要求
画面在原位置出现中文标签，且 PCSX2 texture dump 恰好出现 421 像素集合的
delta；其余四项仍要求目标词消失并精确命中
2,297／2,197／803／1,325 像素集合，才能把对应
`candidate_scene_ids` 升级为正式 member/chunk/record/picture 映射。静态
preview、ISO 构建通过或任意页面变化都不能单独晋级。

五个已筛选候选现均有独立配置和单成员 ISO，其中 chunk 2 已增加中文生产
候选。下一阶段不再新增同类离线 canary，而是逐张完成截图／texture-dump
双门；前一张 atlas 的运行归属不得推断到后一张，任何一张也不得在定位前合入
组合中文生产镜像。
...
- 19 个用例：9 个 UI／路线验收、5 个 001～005 开场序列、1 个中文信息页
  atlas 场景映射／显示实验和 4 个擦除定位实验；
...
8. **已完成五项最小映射 canary 的静态组件和隔离 ISO：**chunk 2 只清除
   `SHIP`，chunk 4 只清除 `COMMAND MENU`，chunk 5 只清除大号 `バザー`，
   chunk 6 只清除幕间标题，chunk 7 只清除 `新規編成`；五者均受
   mask／背景色集合约束，
   完整 KVMDATA 等长、65 个未替换 ISO 成员 byte-exact、零 LBA 位移，并
   分别固定最终 ISO hash。chunk 2 另已生成受审中文 `机体` 候选：
   318 个文字像素、相对原图 421 个像素变化、独立等长 ISO 均已静态锁定。
9. **已完成运行矩阵；当前执行目标：**19 个用例已绑定七张精确 ISO、
   fixture、截图点和断言。先执行四个 fresh-boot 用例；取得并哈希锁定六份
   原生 memory card 后，先用信息页中文候选证明中文标签出现与 421 像素
   texture delta 同时命中，再用其余四张精确 canary ISO 分别证明
   `COMMAND MENU`／`バザー`／幕间标题／`新規編成` 的截图消失与
   2,297／2,197／803／1,325 像素 texture delta 同时命中；另用 UI
   core 精确 ISO 完成标题、玩家设置、战场、搜索和世界史滚动路线，同时按
   官方术语扩大人物／机体名语料。
10. 信息页中文 atlas 只有通过上述运行双门后，才能与前五关 STAGE/HB
