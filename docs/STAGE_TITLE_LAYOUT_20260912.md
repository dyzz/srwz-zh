...
> 同日后续完成的 16 项译名修改见第 6 节，验证记录见第 7、8 节；
> 当前生产构建使用第 9 节的冻结索引快照，不再调用标题渲染器。

## 1. 标题的三条显示链

| 显示位置 | 数据来源 | 本次结论 |
| --- | --- | --- |
| 进关大标题、场间底部跑马灯「第xx話「…」已通关!」 | `DATA/VT1.BIN` group 8 的 107 张 512×64 贴图 | 使用本项目已定稿的冻结索引像素；跑马灯横向滚动，长度只影响滚动时间 |
...
2026-09-12 改为递减阶梯 `[16, 8, 7, 6, 5, 4]`，冻结前的渲染构建按顺序取第一个能装回槽位的等级，
...
按用户要求，107 张标题图已从第 7 节的已验证 Original ISO 直接提取为
[`stage-title-graphics-indexed-snapshot.json`](../config/assets/stage-title-graphics-indexed-snapshot.json)。
冻结过程没有调用渲染器、重采样或重新量化；保存的是每张 512×64 贴图的
4-bit 索引像素，并登记标题 ID／文字、源像素哈希、目标像素哈希、压缩槽哈希及整组哈希。
快照只包含定稿图像平面和审查元数据，TIM2 头、CLUT 和其他原盘数据仍从原盘读取。

生产路径 `_apply_full_stage_title_graphics()` 现在只执行：

1. 验证快照文件锁、107 个 ID／译名与创作排版参数。
2. 检查原盘资源区、指针表和每张图的源像素前像。
3. 解开快照的无损封装，将既有索引像素注入原 TIM2 图像平面。
4. 用生产压缩器装回原槽并回读，核对每槽与整组定稿哈希。

该路径不接收字库或字符映射，不调用 `decode_glyph()` 或 `render_stage_title()`，
也不再尝试其他量化等级。字体文件变化不能改变标题贴图；若文字、创作排版参数、
快照内容或压缩结果漂移，构建直接失败，不会回退到渲染。图形报告登记
`build_mode=locked_indexed_snapshot`、`normal_build_rasterization=false`；
`stage_title_graphics_snapshot` 作为独立输入加入增量依赖图，改变快照会使 VT1 缓存失效。
读档详情和剧情流程底栏仍走 COMPDATA 文本链，此次不改变其控制码策略。

今后只有明确审阅另一份现成 ISO 后才能执行冻结入口，例如：

```bash
python3 tools/freeze_stage_title_graphics.py --refreeze \
  --iso build/iso/zh-release-full-story/current-original.iso
```

工具要求 ISO 与指定构建配置的 SHA-256 相符、VT1 与组件清单相符、每张图与清单
像素哈希相符，才会更新快照和配置锁。它也不会渲染图片；`--refresh-manifests`
等普通构建选项不会调用这个冻结入口。

冻结验证见 [`stage-title-frozen-validation.json`](../manifests/stage-title-frozen-validation.json)：

- 冻结快照为 273,617 字节，SHA-256 `e499dfdf68d2485a2bf88625ee49d01ea2652559f3a7f9d05f45c0b9f0ab520d`。
- 在禁止标题渲染和字形访问的条件下，从原盘回放 107 张图成功；整组哈希为
  `691e50e88711d105d2e34da3cd3b888f692faa3366bd6e2edaf688ed907cc758`，与定稿 ISO 完全一致。
- 使用定稿构建输入进行隔离完整组件重建，19 个输出成员均与冻结前逐字节一致。
  为排除随后独立的 Z 高达名称修改，隔离配置使用 `67f0e27` 的机体名语料副本；
  当前正式语料未改动，隔离报告也未覆盖生产 manifest。
- 全套 302 项测试通过；8 项冻结专项覆盖像素损坏、标题／排版漂移、顺序／数量错误、
  压缩封装异常、生产路径禁止渲染及快照变更使 VT1 缓存失效。
- 当前定稿 ISO 的哈希重新核对一致，本次未重写 ISO、未新运行模拟器，Best 验收仍待完成。
