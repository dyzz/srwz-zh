...
全屏背景、山地景观或普通纹理，没有证据表明它们是第二套文本字库。图片 atlas
中的日文必须单独重绘，扩主字库不会改变这些像素。

另发现 `MAP/MAPNAME.BIN` 的 195 条固定 Shift-JIS 地图名。它们应纳入 code
活性和缺字扫描，但还要通过反汇编/PCSX2 证明实际走本页的主字库 renderer。
完整资产证据见 `ASSET_ANALYSIS.md`。
