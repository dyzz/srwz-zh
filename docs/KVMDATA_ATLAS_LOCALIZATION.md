...
> 技术参考：本文保留图集结构和写回方法；已移除的重冻结脚本属于历史维护流程，
> v0.3.0 普通构建只消费已审核快照。
...
| 5 | 集市等 14 项（英文装饰及原始 `資金` 保留） | `ui-bazaar-atlas-zh.json` | 14 |
...
chunk 5 的小字号状态文字不能用通用中文矢量字体近似；即使 outline/fill 比例
接近，字形骨架和笔画端点仍会与原贴图明显不同。按最终手工验收决定，`資金` 完整
留在全部写回 mask 之外，直接保留日版原贴图；`SR点数・` 只替换确实需要本地化的
`ポイント`，原始 `SR／・` 同样留在写回 mask 之外并逐像素保留。`点／数` 从
原版 VT1 的 24×24 日文字模（glyph 3487／2964）缩放到两个 20×20 字格，再按
原贴图 `1..7`／`8..15` 两层索引固化。仅匹配两层总比例仍不够：暗层索引过高
会使描边在运行画面中变亮，并与填充粘连。`点数` 模板逐层排序后严格匹配源
`ポイント` 的 `1..15` 各索引数量，因此可见像素数、描边／填充数量和每层亮度
直方图均相同；构建链不再为 `資金` 生成或回写任何本地化字形。
