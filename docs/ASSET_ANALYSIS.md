...
节子路线第一关剧情前会显示
`月面　地球連邦軍ルテチウム基地`。曾在 STAGE 0 号 `flow.bin` 流程梗概和
HSFC Scenario Chart 摘要中命中同名词并制作固定跨度候选，但精确 ISO 的运行
截图仍显示原日文，证明这两份文本不是该标题的显示源；对应候选已撤销。
HSFC 已验证的四张 TIM2 也只有流程图界面元素，没有这行标题。

当前证据支持它是独立栅格或尚未定位的渲染资源，但在找到纹理、调色板或实际
运行时 draw source 前不把“栅格”写成定论。固定上游仅提供预制 KVMDATA/KVPDATA
图片替换，并不修改 STAGE 0、HSFC，也没有该世界地图标题的替换文件。
