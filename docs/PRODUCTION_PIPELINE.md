...
### Lessons learned：贴图必须继承原调色板索引语义

所有贴图汉化在动笔前都必须审计原图的逐像素调色板索引，确认背景、描边、填充、
透明别名、选中／未选中等索引层的实际职责。不能只对照导出的 RGBA 颜色，也不能
让 writer 在多个相同 RGBA 的索引中自动选择最小值；相同可见颜色的不同索引可能被
运行时 palette／材质状态解释成完全不同的亮度。

中文重绘必须严格复用目标原文字块的索引分层：各层只能写入原层允许的索引范围，
背景索引、受保护元素及 mask 外像素保持原样。每项写回都应锁定原文字块的 RGBA
前像和索引分布，并用回归测试证明中文描边／填充层非空、非目标字节不变。若尚未
确认索引语义，则该贴图不得进入生产组件。

### Lessons learned：双字节可见文本禁止裸半角空格

使用双字节 glyph 的运行时文本中，语料里的普通英文空格是逻辑分词符，不等于可以
原样写入单字节 `0x20`。裸 `0x20` 会改变后续字节的配对：例如逻辑文本
`Anti Earth Union Government` 中，第一个空格会让 `Earth` 被两两组合成无关汉字，
直到下一个单字节空格才恢复配对。不能通过删除空格、翻译英文标识或修改字形来掩盖
这个编码错误。

语料和术语比较继续使用普通 ASCII 空格；writer 必须在最终排版完成后、编码写回前，
把可见空格转换为原版双字节空格 glyph `0x8140`。换行、格式 token 和控制码仍按各自
语义编码，不得统一替换。独立回读要把 `0x8140` 归一化回普通空格再与逻辑文本对账，
同时扫描实际字段 payload，发现裸 `0x20` 立即失败。

该门必须覆盖一个 surface 的全部运行副本，不能只修截图中的单条文本。Key Word
至少包括 `MTVZKNKW.BIN` 的 52 条、每条 `WORD/SRCE/DSCR/DSC2` 四个字段，
COMPDATA 的 52 个列表名和 STAGE 的 77 份剧情弹窗副本；资料库权威、复制后的目标
字节和最终 ISO 回读都必须通过同一检查。静态通过只证明存储字节正确，精确 ISO 上的
PCSX2 目标画面仍是独立运行验收门。
