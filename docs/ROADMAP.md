...
候选：chunk 2 `机体`、chunk 4 `指令菜单`、chunk 5 `交易所`、chunk 6
`中场休息`、chunk 7 `新建小队`。相对原图分别变化
421／2,292／3,634／2,083／1,262 个像素；完整归档等长，五张单成员 ISO
都有 65 个未替换成员和零 LBA 位移。旧 canary 的像素集合、mask 和容器
golden 继续保留为定位前像证据；五张中文候选的截图／texture dump 运行双门
仍未完成。
世界史 28 条已完成
22 格中文断行、14 个空行
...
- P0 场景选择、译文决策、动态名称结构和独立统一字库候选已建立机器门禁；
  ...
  researched 精确名称也已完成追加字库、writer 和 P2 组合 ISO。五张已筛
  KVMDATA atlas 均有独立中文候选；其运行归属、与 P2／前五关 STAGE/HB
  的合并和逐屏运行验证待完成。
- 命令、按钮和战斗条件。
- 精神、技能和特殊能力。
- 零件、武器、机体、驾驶员和关卡名。
