...
当前只缺九个汉字，能放入剩余 12 个追加槽并保留三槽余量；这只是输入和容量
门禁，不代表真实 SLPS/COMPDATA 池、动态名称、信息页 atlas、组合 ISO 或
PCSX2 路线已经完成。具体实施和验收见
...
- P0 场景选择、译文决策、字形需求和动态名称探针已建立机器门禁；
  真实批量 writer、统一字体重绘、组合 ISO 和逐屏运行验证待完成。
- 命令、按钮和战斗条件。
- 精神、技能和特殊能力。
- 零件、武器、机体、驾驶员和关卡名。
- 图鉴和所有图像内嵌文字。
