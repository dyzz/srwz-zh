...
组件；标题四项、462 条 P0 文本、开场 45 项加 researched 1,262 项、
P2 字库和世界史已通过无冲突组合，生成静态验证的 `ui-p2-core` ISO。
...
PCSX2 逐屏证据，也不包含前五关 STAGE。信息页 `SHIP` 定位切片现已进一步
生成首个受审中文 `机体` 图集候选：318 个文字像素都在原 `49×16` mask 内，
相对原图精确变化 421 个像素；独立 ISO 只替换等长 `KVMDATA.BIN`，65 个
其余成员 byte-exact、零 LBA 位移。战场 `COMMAND MENU`、商店 `バザー`、
幕间标题与编成 `新規編成` 仍各有一张只擦除标签的隔离映射 canary。五项都
尚无截图／texture dump 双证据，因此信息页中文候选仍是
`runtime_mapping_pending`，其余四项也不计为正式运行映射或中文 atlas。
