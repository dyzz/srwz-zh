#!/usr/bin/env python3
"""Rebuild and verify the configured combined localized UI atlas."""
...
def main() -> int:
    ...
    if not archive_path.is_file():
        raise SystemExit(
            f"atlas suite is missing; run build_ui_atlas_suite.py: {archive_path}"
        )
    ...
    if args.refresh_manifest:
        ...
        if not manifest_path.is_file():
            raise SystemExit(
                "atlas suite manifest is missing; review and use "
                "--refresh-manifest"
            )
        committed = json.loads(manifest_path.read_text(encoding="utf-8"))
        if committed != report:
            raise SystemExit(
                "atlas suite manifest drift; review and use --refresh-manifest"
            )
