#!/usr/bin/env python3
"""Verify the incremental P0 UI font component and committed manifest."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Reparse the generated UI P0 SLPS/VT1 component, audit all "
            "selected UI glyphs and verify its byte-free manifest."
        )
    )
...
def main() -> int:
    ...
    if args.refresh_manifest:
        ...
        try:
            ...
            raise SystemExit(
                f"cannot load UI P0 font manifest {manifest_path}: {error}"
            ) from error
        if manifest != report:
            raise SystemExit(
                "UI P0 font manifest drift; review the local report, then "
    ...
    print(
        "UI P0 font:",
        f"entries={report['p0_renderer_coverage']['unique_entry_count']}",
        f"missing={report['p0_renderer_coverage']['missing_renderer_character_count']}",
