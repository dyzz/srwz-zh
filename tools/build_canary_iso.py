#!/usr/bin/env python3
"""Build and statically validate the SRWZ Chinese canary PS2 DVD image."""
...
class IsoBuildError(RuntimeError):
    """The ISO build cannot meet its pinned input or output contract."""
...
def validate_directory_contract(config: dict) -> None:
    ...
    if Path(profile_id).name != profile_id:
        raise IsoBuildError("ISO build profile_id must be one path segment")
    ...
    for index, replacement in enumerate(replacements):
        if not isinstance(replacement, dict):
            raise IsoBuildError(f"replacement {index} must be an object")
        _require_config_path_under(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a coherent ISO9660/UDF SRWZ canary PS2 DVD image."
        )
    )
