#!/usr/bin/env python3
"""Explicitly render and freeze the runtime NISVDATA sound-select title."""
...
try:
    ...
    from srwz.nisv_library_menu import build_nisv_sound_select
except ModuleNotFoundError:
    from tools.srwz.font_flavor import (
        load_font_flavor_reference,
        verify_font_flavor_files,
    )
    from tools.srwz.nisv_library_menu import build_nisv_sound_select
...
DEFAULT_OUTPUT = (
    PROJECT_ROOT / "config/library/sound-select-runtime-render-snapshot.json"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Explicitly rerender and freeze the runtime sound-select title."
    )
...
def main() -> int:
    ...
    contract = config["sound_select_runtime_tim2"]
    ...
    snapshot: dict = {}
    _output, report = build_nisv_sound_select(
        source,
        contract,
    ...
    preview = WORK_ROOT / "refreeze/sound-select/sound-select-runtime.png"
